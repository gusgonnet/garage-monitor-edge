...
void BTHome::toJson(JSONWriter *writer) const
{
    ...
    writer->name("packetId").value(getPacketId());
    writer->name("batteryLevel").value(getBatteryLevel());
    writer->name("buttonEvent").value(getButtonEvent());
    writer->name("windowState").value(getWindowState());
    writer->name("rotation").value(getRotation());
    writer->name("illuminance").value(getIlluminance());
    writer->endObject();
}
